...
def register():
    ...
    # Using .get() is safer than data.values() to ensure order matches SQL
    values = (data.get('full_name'), data.get('student_id'), data.get('email'), 
              data.get('phone'), data.get('department'), data.get('year'), 
              data.get('event_type'), data.get('event_name'), data.get('category'), 
              data.get('team_size', 1), data.get('notes'))
    cursor.execute(sql, values)
    conn.commit()
    cursor.close()
    conn.close()
    socketio.emit('registration:new') 
    return jsonify({"status": "success", "full_name": data.get('full_name')}), 201
...
def delete_registration(id):
    ...
    cursor.close()
    conn.close()
    socketio.emit('registration:deleted')
